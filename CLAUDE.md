...
- `consult_` — Freshman Global free strategy & EC consultations (subscription giveaway, stateless — no flow, no DB table). Menu button (shares row 1 with merch) or `consult_open` inline button → `CONSULT_INTRO` promo → membership gate on @freshmanglobal + @freshmanblog (`CONSULT_REQUIRED_IDS` in bot.py — @freshmanglobal is still an @handle awaiting its numeric ID; the bot must be an **admin** in both channels or the check fails open, guidebook-style) → pass → `CONSULT_ACCESS_GRANTED` with booking URL buttons (`CONSULT_BOOKING_LINKS`: Hasan + Imron; no Umid link yet). `/broadcastkeyboard` currently sends `CONSULT_INTRO` with a `consult_open` button.
