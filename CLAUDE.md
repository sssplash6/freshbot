...
- `fireside_` — Fireside Chat event registration (Freshman Research Institute). Menu button (shares row 1 with merch) or `fireside_open` inline button → `FIRESIDE_INTRO` → full name → upserted into `fireside_registrations`, Person X notified; `/fireside_list` shows registrations. Per event, swap the `FIRESIDE_*` strings. `/broadcastkeyboard` currently sends `FIRESIDE_INTRO` with a `fireside_open` button.
